...
describe("file loader (index)", () => {
  it('should load lines', async () => {
    assert.equal((await readLinesAsync(DATA_PATH, READ_OPTIONS)).length > 0, true);
  })

  it('should load lines as array', async () => {
    assert.equal(Array.isArray((await readLinesAsync(DATA_PATH, READ_OPTIONS))), true)
  })

  it('should load lines as string', async () => {
    assert.equal(Array.isArray((await readLinesAsync(DATA_PATH, {eol: false}))), false)
  })
